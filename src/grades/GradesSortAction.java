...
public class GradesSortAction extends Action {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int n = 0;
		for (int i = 0; i < testList.size()/8; i++) {
			list.add(testList.subList(n, n+8));
			n += 8;
		}
		if (order.equals("asc")) {
			try {
	    ...
	    for (int i = 0; i < list.size(); i++) {
			out.print("{\"student_id\":\"" + list.get(i).get(0) + "\",\"student_name\":\"" + list.get(i).get(1) + "\",\"student_kana\":\"" + list.get(i).get(2) + "\",\"school_code\":\"" + list.get(i).get(3) + "\",\"class_number\":\"" + list.get(i).get(4) + "\",\"subject_name\":\"" + list.get(i).get(5) + "\",\"attempt_number\":\"" + list.get(i).get(6) + "\",\"score\":\"" + list.get(i).get(7) + "\"}");
	        if (i < list.size() - 1) {
	        	out.print(",");
	        }
	    }
